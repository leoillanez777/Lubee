...
public class TipoPropiedadService(Context context, IMapper mapper) : ITipos {
  ...
  public async Task<ResponseDTO<List<TipoDTO>>> GetAll() {
    ...
    try {
      var data = await context.TiposPropiedades.ToListAsync();
  ...
  public async Task<ResponseDTO<TipoDTO>> GetTipo(int id) {
    ...
    try {
      var data = await context.TiposPropiedades.FindAsync(id);
  ...
  public async Task<ResponseDTO<TipoDTO>> Post(TipoDTO data) {
    var response = new ResponseDTO<TipoDTO> { Success = false };
    try {
      var tipo = mapper.Map<TipoPropiedad>(data);
      if (tipo.Id == 0) {
        context.Add(tipo);
      }
      else {
  ...
  public async Task<ResponseDTO<bool>> Delete(int id) {
    ...
    try {
      TipoPropiedad tipoToDelete = new() { Id = id };
